...
# Pages extracted per semantic chunk.
PAGES_PER_CHUNK = 5

# Hard cap on stored text per chunk.
...
class UploadService:
    ...
    def complete_upload(self, upload_id: str, background_tasks: BackgroundTasks) -> dict:
        ...
        # Process in the API process — no separate worker needed
    ...
    # ------------------------------------------------------------------
    # Processing (runs as a background task in the API process)
    # ------------------------------------------------------------------
    ...
    def _process_upload(self, upload) -> None:
        ...
        Strategy
        --------
        1. Download the PDF from B2 to a temporary local file (streaming).
        2. Open with PyMuPDF (lazy page access, low memory).
        3. Extract PAGES_PER_CHUNK pages at a time, upload text to B2 as
           .txt files, and store only the B2 key in Postgres.
        4. Free memory after every chunk.
        """
        ...
        try:
            ...
            for start_page in range(0, total_pages, PAGES_PER_CHUNK):
                ...
                # Sanitise: strip null bytes and non-printable chars
                ...
                # Save text to B2 (not Postgres)
                r2_text_key = f"text/{upload.id}/chunk_{chunk_idx}.txt"
                upload_text(r2_text_key, text)

                # Save chunk metadata to Postgres (lightweight)
                chunk_record = self.upload_repo.save_chunk_record(upload.id, chunk_idx, r2_key)
                self.search_repo.save_chunk_text_key(chunk_record.id, r2_text_key)
            ...
            doc.close()
            logger.info("Extraction done. %d text chunks saved to B2.", chunk_idx)
